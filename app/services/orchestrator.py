...
class Orchestrator:
    async def process_chat_stream(self, session_id: str, user_message: str):
        ...
        # 3. Ensure current message is in history
        if not history or history[-1]["content"] != user_message:
            history.append({"role": "user", "content": user_message})

        messages = [{"role": "system", "content": config["system_prompt"]}] + history
        
        # --- PROOF OF MEMORY IN LOGS ---
        print(f"[ZenoAi MEMORY CHECK] Sending {len(messages)} messages to AI. It SHOULD remember everything.")
        # -------------------------------
